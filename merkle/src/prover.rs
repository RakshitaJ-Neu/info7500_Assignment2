...
pub fn gen_merkle_proof(leaves: Vec<String>, leaf_pos: usize) -> Vec<Hash32Bytes> {
    ...
    for level in 0..height {
        // If the position is even, the node has a right sibling
        if level_pos % 2 == 0 && level_pos < state.len() - 1 
        {
            hashes.push(state[level_pos + 1]);
        }
        else if level_pos % 2 != 0 
        {
            hashes.push(state[level_pos - 1]);
        }
        level_pos /= 2;
        ...
        if state.len() > 1 
        {
            state = (0..state.len()).step_by(2).map(|i| {
                let right = if i + 1 < state.len() { state[i + 1] } else { state[i] };
                hash_internal(state[i], right)
            }).collect();
        }
    }
    println!("\n merkel_root {}", encode_hash(state[0]));
